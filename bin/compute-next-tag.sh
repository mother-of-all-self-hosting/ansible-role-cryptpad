...
#
# Tags look like `v<CryptPad version>-<release>`:
#
# - if defaults/main.yml points at a CryptPad version that has never been
#   released, the release counter restarts at 0 (`v2026.5.1-0`)
# - otherwise the counter is incremented (`v2026.5.1-1`), but only if something
...
# `cryptpad_version` is the leaf value: it holds a bare `2026.5.1`, and it is
# the one Renovate bumps. Everything else is derived from it in Jinja
# (`cryptpad_container_image_tag` re-adds the registry's `version-` prefix,
# `cryptpad_container_image` composes the full reference), so none of those can
# be read here. The anchored match keeps the longer variable names that also
# end in `_version` out of the way.
version="$(sed -nE 's|^cryptpad_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the CryptPad version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `2026.5.1`), but the tags
# do (`v2026.5.1-0`). Stripping any `v` before prepending one keeps this
